...
//const char* ssid = "Infinix HOT 10";
//const char* password = "infinixhot13";
...
void setup() {
  ...
  gateServoKeluar.write(5);
  server.on("/open_servo", HTTP_GET, []() {
        gateServo.write(180); // Open position
        delay(4000); // Hold open for 1 second
        gateServo.write(5); // Close position
    ...
    server.on("/open_servo_exit", HTTP_GET, []() {
        gateServoKeluar.write(180); // Open position
        delay(4000); // Hold open for 1 second
        gateServoKeluar.write(5); // Close position
...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    //    Serial.println("Berhasil konek kedalam jaringan");

    String Link;
    HTTPClient http;
    Link = "http://" + String(host) + "/parking/rest.php?uid=" + StrUID;
    ...
    String load_data;
    HTTPClient httpLoadData;
    load_data = "http://" + String(host) + "/parking/api/api_load_data_kendaraan.php?uid=" + StrUID;
    httpLoadData.begin(client, load_data);

    int httpDataLoad = httpLoadData.GET();
    String responseLoadData = httpLoadData.getString();
    Serial.println(responseLoadData);
    //    Serial.println(StrUID);
    delay(200);
    httpLoadData.end();

//    String load_data_kendaraan;
//    HTTPClient httpLoadDataKendaraan;
//    load_data_kendaraan = "http://" + String(host) + "/parking/api/api_load_data_kendaraan.php?uid=" + StrUID;
//    httpLoadDataKendaraan.begin(client, load_data_kendaraan);
//
//    int httpDataLoadKendaraan = httpLoadDataKendaraan.GET();
//    String responseLoadDataKendaraan = httpLoadDataKendaraan.getString();
//    Serial.println(responseLoadDataKendaraan);
//    //    Serial.println(StrUID);
//    delay(200);
//    httpLoadDataKendaraan.end();

    //baca status servo
//    String linkServo;
//    HTTPClient httpServo;
//    linkServo = "http://" + String(host) + "/parking/openGate.php?uid=" + StrUID;
...
//    LinkKeluar = "http://" + String(host) + "/parking/rest.php?uid=" + StrUID;
...
//    httpKeluar.end();
    ...
    linkServoKeluar = "http://" + String(host) + "/parking/open_gate_exit.php?uid=" + StrUID;
