...
if (isset($_POST['uid']) && isset($_POST['slot'])) {
  # code...
...
$sql_check_uid = "SELECT * FROM tb_kendaraan WHERE uid = '$uid' LIMIT 1";
...
if ($result_check_uid->num_rows > 0) {
    while ($result_slot=$result_check_uid->fetch_assoc()) {
      ...
      // $slot_available = $result_slot['slot_available'];
    ...
    $sql_update_slot = "UPDATE tb_slot SET uid='$uid', entry_time='$entry_time', slot_available='$slot', plat_nomor='$plat_nomor', status_slot='terisi', status='180' WHERE id_slot=$slotId LIMIT 1";
